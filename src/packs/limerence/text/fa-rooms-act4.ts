...
registerAll('v2', 'fa', {
  ...
  [roomBeatKey('the-kitchen-table', 0, 2)]:
    'دو صندلی، یک میز. گوشی حالا رو به بالا، بینِ شما، بی‌اهمیت افتاده — هر معنایی که زمانی داشت، تمام شده و رفته.',
  ...
  [roomChoiceOutcomeKey('the-kitchen-table', 'separate-well', 0)]:
    'فاجعه‌ای بی‌درام: لجستیک، در نقشِ مرثیه. یک ضربانِ کامل فقط تقویمِ تحویل‌دادنِ بچه‌هاست، و به‌نحوی سخت‌ترین بخش همین است.',
  [roomChoiceOutcomeKey('the-kitchen-table', 'separate-well', 1)]:
    'آنچه بچه‌ها با خودشان حمل می‌کنند، پژوهش روشن است، به میزانِ تعارض بستگی دارد — نه به دسته‌بندیِ جداشدن یا نشدن. سوگواری، اینجا، در روشناییِ روز، عمداً انجام‌شده.',
  ...
  [roomChoiceOutcomeKey('the-kitchen-table', 'attempt-repair', 0)]:
    'هیچ مونتاژی نیست، هیچ تضمینی نیست — اتاق فقط هفته‌ی اول را نشان می‌دهد: فرمِ پذیرش، نخستین بازبینیِ صادقانه، رابطه‌ی نامشروع (مالِ هرکدامتان که بود) بررسی‌شده به‌عنوانِ زنگِ خطر، نه فقط جرم.',
...
registerAll('v2', 'fa', {
  ...
  [roomChoiceOutcomeKey('the-unsent', 'to-the-one-that-got-away', 0)]:
    'نامه‌ای که آدم‌های صادق از آن می‌ترسند، بیشتر به‌خاطرِ چیزی که ممکن است اصلِ نوشتنش، به‌تنهایی، معنا بدهد.',
...
registerAll('v2', 'fa', {
  ...
  [roomChoiceHintKey('the-morning-desk', 'stand-by-all')]: 'ثبات، پذیرفته‌شده',
  ...
  [roomChoiceOutcomeKey('the-morning-desk', 'name-what-changed-me', 1)]:
    'دستِ دربان، روی دفترحساب، مکث می‌کند — و برای اولین‌بار در کلِ امشب، حلقه‌ی ازدواج را روی دستِ راستش می‌بینید، و آن خطِ کم‌رنگ و آفتاب‌نخورده‌ای را جایی که یک‌بار، حلقه‌ای روی دستِ چپش بود. «این همان جوابی است که من برایش اینجا هستم،» می‌گوید، بدونِ آنکه سر بلند کند. «بازنگری، همراه با رسید. کمیاب‌تر، و بهتر، از ثبات.»',
  ...
  hasFlag(s, 'confessed-whole')
    ...
    : hasFlag(s, 'carried-alone')
      ...
      : hasFlag(s, 'trickle-truth')
        ...
        : 'دربان: «هیچ اعترافی هیچ‌جای این پرونده نیست. یا اصلاً بدهکارِ هیچ اعترافی نبودید، یا یکی هنوز، همین امروز صبح، ثبت‌نشده مانده.»',
);

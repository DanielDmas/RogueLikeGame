...
**7. Door hover feedback beyond the shipped pulse.** `doors.ts` already has a breathing glow pulse, a per-door pitched hover tone, and a DOM tooltip with the door's hint/teaser. A richer treatment (hover-triggered light-spill increase, a subtle door-creak sound) is real but speculative production-taste work, not a gap — kept open at low priority.
...
**10. F5 Tier 3 — LIMERENCE dioramas beyond Tier 1+2.** 18 of 34 LIMERENCE rooms now have bespoke dioramas (Tier 1) and 6 of those have per-choice accent hooks (Tier 2, commit `bf83fbc`). The remaining ~16 rooms keep their floor theme — acceptable once every act reads distinctly (already true), but a full parity pass with ANAMNESIS's ~22 would be more.
